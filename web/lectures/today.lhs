...
\begin{code}
tot3 (IOneAndMore x (IOneAndMore y (IOneAndMore z _))) = x + y + z
\end{code}
...
val :: IntTree -> Int
val (ILeaf x)   = x 
val (INode l r) = error "*** Exception: No match in record selector val" 

left :: IntTree -> IntTree
left (ILeaf _)   = error "YIKES"
left (INode l _) = l
...
ftail :: Fist a -> Fist a
ftail (Con _ xs) = xs
ftail _          = error "DONT KNOW"



data Weird a b = WE | WW a (Weird b a)


type Blob = Weird Int Bool

w0, w1, w2 :: Blob -- , w3, w4, w5, w6 :: Blob
w0 = WE
w1 = WW 0 WE
w2 = WW 1 (WW True (WW 2 (WW False WE)))

type Goo = List (->)
-- fs = Con 1 (Con 2 (Con 3 Emp))

-- *Main> :t (ftail (ftail (ftail fs)))
...
pat b op (Leaf x)   = b x
pat b op (Node l r) = (pat b op l) `op` (pat b op r)

height = pat (\_ -> 0) (\vl vr -> 1 + max vl vr)
size   = pat (\_ -> 1) (vl + vr)
~~~~~
